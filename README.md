...
1. React에서 기본으로 제공하는 상태 관리 도구로, 외부 라이브러리 설치 없이 사용 가능

2. Context API는 주로 전역 상태를 관리하는 데 사용합니다.

3. React.createContext()로 생성한 Context 객체와 Provider 컴포넌트를 사용해 상태를 하위 컴포넌트에 전달합니다.

### Context API 장점

1. 간단하고 가볍다 : 외부 라이브러리 설치 없이 기본 React 기능만으로 전역 상태 관리를 할 수 있습니다.

2. 적은 설정 필요 : 간단한 구조를 가지고 있어 설정과 사용이 간편합니다.

3. 컴포넌트 트리의 길이 제한 없음 : 여러 단계에 걸쳐 상태를 전파할 수 있어 prop drilling 문제를 해결합니다.

### Context API 단점

1. 복잡한 상태 관리에 한계 : 상태가 복잡하거나 다양한 액션을 통해 변경이 이루어져야 하는 경우, 관리가 어려워질 수 있습니다.

2. 성능 문제 : 상태가 업데이트되면 해당 상태를 사용하는 모든 하위 컴포넌트가 다시 렌더링되므로, 상태 범위가 넓을 경우 성능에 영향을 미칠 수 있습니다.

3. 디버깅 도구 부족 : 상태 변경 과정을 추적하고 관리하는 Redux DevTooks와 같은 도구가 기본적으로 제공되지 않습니다.

### Redux 단점

1. 설정과 코드 복잡도 : Context API에 비해 설정이 복잡하며, boilerplate 코드가 많이 필요합니다.

2. 추가 라이브러리 필요 : Redux 자체가 외부 라이브러리이므로 설치 및 유지 관리가 필요합니다.

3. 작은 애플리케이션에는 과한 설정 : 단순한 상태 관리가 필요한 작은 애플리케이션에서는 과도한 설정일 수 있습니다.
